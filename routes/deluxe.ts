...
module.exports.upgradeToDeluxe = function upgradeToDeluxe () {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      const user = await UserModel.findOne({ where: { id: String(req.body.UserId), role: security.roles.customer } })
      if (user == null) {
      ...
      if (req.body.paymentMode === 'card') {
        const card = await CardModel.findOne({ where: { id: String(req.body.paymentId), UserId: String(req.body.UserId) } })
